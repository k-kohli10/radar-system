...
every time: find the right runbook, correlate it with the other alerts that just fired,
figure out what changed recently, and decide where to start looking. That triage work is
repetitive, time-pressured, and rarely captured anywhere afterward.

RADAR automates that first ten minutes. It does not replace the engineer's judgment —
it gets them to an informed starting point faster, with a documented trail of what was
...
- **Not a detection system.** Prometheus alerting rules and Kibana Watcher decide when
  something is wrong. RADAR only acts on alerts they have already fired.
...
- **Not a general-purpose agent framework.** The agent pipeline (Watcher → Planner →
  Reasoner) is a fixed, three-stage sequence purpose-built for incident triage — not a
  platform for arbitrary agent workflows.
- **Not a ticketing system.** Incident state lives in Postgres. There is no Jira/ServiceNow
  integration.
...
written to a Postgres outbox table, picked up by a dedicated outbox worker, and dispatched
with retries and idempotency guarantees. See
...
memory pressure — drive the alert scenarios, runbooks, and demo narrative. See
[docs/architecture/system-overview.md](docs/architecture/system-overview.md).

## Status

RADAR is built incrementally, one phase at a time, each phase landing as its own PR.
