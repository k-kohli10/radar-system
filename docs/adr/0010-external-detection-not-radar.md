...
`radar-infra/prometheus/alerting-rules.yml` — config, not RADAR application code. RADAR
receives only pre-fired alerts, via `POST /alerts/prometheus` and `POST /alerts/kibana`
on the ingestion service. RADAR's job starts at correlation, not detection.
