...
- "State changed but the event was never sent" and "event sent but the state change
  never committed" are both structurally impossible — they're the same transaction.
...
- `FOR UPDATE SKIP LOCKED` lets multiple outbox-worker replicas poll concurrently
  without double-claiming the same event — horizontal scaling of the dispatcher is
  free.
