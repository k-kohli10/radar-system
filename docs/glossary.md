...
gateway: incident metadata, investigation steps, and (from Phase 8 onward) retrieved
runbook context.
...
call before being handed to the reasoner; `insufficient` chunks are dropped.

**Dead letter** — An outbox event that exhausted all retry attempts. Stops retrying
...
scoped to exactly one mode, which maps to a specific provider/model/token-limit/timeout
configuration.
...
**RCA** — Root Cause Analysis. The reasoner's structured output: a root cause narrative,
a confidence level, and recommended actions.
