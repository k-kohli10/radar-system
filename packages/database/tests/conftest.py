...
LOCKED``, unique constraints — so they need a running Postgres. The URL comes
from ``POSTGRES_TEST_DSN`` if set, otherwise it is derived from ``POSTGRES_DSN``
(routed to a dedicated ``radar_test`` database so dev data is never touched),
otherwise read from the repo-root ``.env``. If none is available, or Postgres is
unreachable, the suite is skipped.

The schema is built once per session with ``create_all`` (verified elsewhere to
match the Alembic migration), and every table is truncated before each test.
"""
