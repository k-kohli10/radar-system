...
The outbox worker delivers at *least* once, so before any interpretation of the event
at all this handler asks ``processed_events`` whether this ``event_id`` has already
been handled by ``feedback-service``. If it has, the answer is 200 and nothing else
happens.

This gate is the EVENT-level idempotency contract every RADAR handler keeps. The
no-double-post guarantee is a separate mechanism: once a recommendation's card is
delivered its ``slack_message_ts`` is set, and
:func:`~radar_feedback_service.delivery.deliver_rca` skips on that under a row lock
regardless of this gate. The two are complementary.

``recommendation.created`` is delivered by :func:`deliver_rca` inside ONE transaction
the handler commits: the ``slack_message_ts``, the delivery audit row, and the
processed marker land together or not at all. The Slack post happens inside that
transaction, holding the recommendation's row lock across it; see the delivery module
for why that is right here and wrong in the reasoner.

Failure mapping is the outbox contract:

- The post failed (:class:`NotificationFailedError`) -> **503, unmarked**. The card
  was not delivered and the transaction rolled back, so redelivery retries. A failed
  post is never recorded as done.
- A missing recommendation or incident (:class:`~radar_common.NotFoundError`) ->
  **422**. The reasoner writes the recommendation in the same transaction as the
  event, so a missing one is corruption: dead-letter it to a human rather than retry
  a fault that will not clear.

An event type belonging to a DIFFERENT service takes the standard mark-and-drop: an
...
def create_events_router(
    *,
    ...
    ``get_database`` and ``get_notifier`` return the live database and Slack backend
    (set during startup) or ``None`` when the service is not ready, in which case the
    handler answers 503 rather than touching a dependency that is not there.
    ...
    async def handle_event(envelope: EventEnvelope) -> dict[str, str]:
        ...
        async with database.session() as session:
            ...
            # all. Event-level idempotency; the double-post guard is the delivery
            # lock (see the module docstring).
            ...
            try:
                ...
                # A missing recommendation/incident is corruption: permanent, so
                # dead-letter it to a human rather than retry forever.
                log.error(
                ...
                # The post failed. Nothing was recorded (the transaction rolls back on
                # the way out), so redelivery retries: at-least-once. Retryable 503,
                # deliberately NOT marked processed.
                log.warning(
                    "rca.delivery_failed",
                    event_id=str(envelope.event_id),
                    reason=str(exc),
...
def _parse_payload(envelope: EventEnvelope) -> RecommendationCreatedPayload:
    ...
    A widened or wrong-shaped payload means the reasoner and this service disagree on
    the contract: a permanent fault, so it dead-letters rather than retrying.
    """
    try:
