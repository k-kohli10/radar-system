...
The outbox worker's guarantees are Postgres guarantees: ``FOR UPDATE SKIP LOCKED``
row locking, transaction-scoped ``NOW()``, and real concurrent transactions. None
of that exists against SQLite or a mock, so these tests need a running Postgres
and skip when one is not available.

DSN resolution mirrors the ``radar_database`` and ``radar_ingestion`` suites
exactly (so all three skip/route identically): ``POSTGRES_TEST_DSN`` if set,
otherwise derived from ``POSTGRES_DSN`` (routed to a dedicated ``radar_test``
database so dev data is never touched), otherwise read from the repo-root
``.env``. The schema is built once per session with ``create_all`` and every
table is truncated before each test.
"""

from __future__ import annotations

# NOTE: this is the THIRD copy of this DSN resolution / schema build / truncation
# block (packages/database, apps/ingestion, and now here). The ingestion copy says
# to extract a shared testing helper once a third consumer appears — that is now.
# Deliberately left duplicated in this commit so the concurrency proof lands
# unmixed with a cross-package test-infra refactor; the extraction is its own
# change.
import asyncio
import os
