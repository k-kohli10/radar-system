...
2. **A representative pipeline latency.** `radar_incident_duration_seconds` — the
   ...
   once per recommendation, over the incident's `opened_at → recommendation
   created_at` span (both Postgres `now()`, one clock). The 100 alerts queue
   through the outbox, so the histogram gets a real p50/p95/p99 distribution rather
   than Phase 10's single best-case point. The test asserts the histogram recorded
   all 100 observations and prints the percentiles.
