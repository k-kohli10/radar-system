...
and hands the mention text over exactly as Slack sent it. Stripping the bot handle and
matching verbs is the app's job.
...
delivers only THIS app's own interactions and events over that connection, so the
authenticated socket IS the authentication. There is deliberately no request-signature
check here: signing (``X-Slack-Signature``) is the HTTP Events API's mechanism, needed
because a public HTTP endpoint loses the property Socket Mode gives for free. When
Phase 12 moves to Events API + ingress, signing becomes mandatory.
...
since the envelope is already acked by the time it runs (see below)."""

MentionHandler = Callable[[BotMention], Awaitable[None]]
"""What the app registers for ``@radar`` mentions: handles one mention. Like the
interaction handler, it runs after the envelope is acked."""
...
def to_interaction(payload: dict[str, Any]) -> NotificationInteraction | None:
    ...
    identifying field: the neutral contract is always complete or absent, so the
    app never has to defend against a half-empty one.
    ...
    given (``None`` if the control had none); deciding whether a value is required
    and validating it is the app's job.
    """
    ...
    if not (action_id and user_id and channel_id and message_ts):
        ...
        # be acted on or replied to, so it is dropped rather than forwarded.
...
def to_mention(payload: dict[str, Any]) -> BotMention | None:
    ...
    identifying field, the same rule :func:`to_interaction` follows. The events-API
    envelope wraps the event under ``event``; only that inner ``app_mention`` is a bot
    command surface.

    ``text`` is passed through AS SLACK SENT IT, still carrying the leading ``<@BOT>``
    handle: stripping and parsing are the app's job, not the transport's. ``ts`` (the
    mention message's timestamp) becomes the thread the reply is posted under.
    """
    ...
    if not (text and user_id and channel_id and message_ts):
        # A mention missing its identity/routing fields cannot be replied to, so it
        # is dropped rather than forwarded, as with block_actions above.
...
async def ack_and_dispatch(
    ...
    ``events_api`` ``app_mention`` to ``mention_handler``. Anything else, including a
    mention when no ``mention_handler`` is registered, is acked and dropped.

    The ack goes FIRST, before any work, inside Slack's ~3s window, because an
    unacked envelope is redelivered. Processing therefore happens after the ack, so a
...
class SlackSocketSource:
    ...
    dropped, so a deployment can wire the interaction side alone.
    """
    ...
    def __init__(
        ...
        *,
        ...
        given, each translated ``@radar`` mention."""
